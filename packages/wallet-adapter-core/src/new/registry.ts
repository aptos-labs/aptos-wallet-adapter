...
import { WalletIcon } from '@aptos-labs/wallet-standard';
import { AptosStandardWallet } from './AptosStandardWallet';

export interface DiscoverableWallet {
  name: string;
  url: string;
  icon: WalletIcon;
...
export const petra: DiscoverableWallet = {
  name: "Petra",
...
export const pontem: DiscoverableWallet = {
  name: "Pontem Wallet",
...
export const nightly: DiscoverableWallet = {
  name: "Nightly",
...
export interface DefaultRegistryConfig {
  /**
   * Aptos Connect is included by default.
   * You can pass a custom configuration, or `false` to disable it.
   */
  aptosConnect?: AptosConnectWalletConfig | false;
}

export type RegistryWallet = DiscoverableWallet | AptosStandardWallet;

/**
 * Default registry of AIP-62 wallet standard wallets.
 * This list is used to show supported wallets even if they are not installed on the user machine.
 */
export function makeDefaultRegistry({ aptosConnect = {} }: DefaultRegistryConfig = {}): RegistryWallet[] {
  const isSSR = typeof window === "undefined";
  const aptosConnectWallets = !isSSR && aptosConnect ? [
    new AptosConnectGoogleWallet(aptosConnect),
    new AptosConnectAppleWallet(aptosConnect),
  ] : [];
  return [
    ...aptosConnectWallets,
    petra,
